...
- [ ] 滑鼠停在材料明細或刷圖計畫的關卡列上，會彈出這關掉落的所有（本站範圍內的）材料與單次期望值；移開會消失，不會卡住
- [ ] 按「刷圖計畫」：有需求時列出建議合成（含遞迴多層，例如 A 合成要 B、B 合成要 C）與建議刷關（含同一關同時列出好幾個材料的情況）；沒有已知取得方式的材料會列在「無法排進計畫」，不會讓整個計畫消失或報錯；點材料清單裡任一材料會切回材料明細，不會跟計畫畫面疊在一起
