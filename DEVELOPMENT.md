...
- [ ] 技能等級列是單一一排（1-7 接著專Ⅰ-Ⅲ，共 10 顆，不會換行、不會多出「無」按鈕）；普攻分頁也看得到、也能點整排——專精按鈕在普攻分頁下不是灰的，點下去會改到「最後造訪的技能分頁」的專精，切去那個分頁能看到剛剛的改動確實生效
...
- [ ] 「設為初始」「設為滿級」在有編輯中的分頁時，也會直接寫進那份養成狀態（不是單純預覽）；「設為滿級」的潛能維持潛 1
- [ ] 技能等級按鈕：選一個技能分頁後，先點某個專精（Ⅰ/Ⅱ/Ⅲ）反白，再點回 1-7 的任一等級，反白要換成剛點的那個等級，不能停在原本的專精上不動（這是實際修過的 bug，不是憑空的測項）
- [ ] 養成狀態摘要格式是「精X／LvY→精X／LvY　技能：Z→Z」，技能獨立列在後面、不是併進 `精X／LvY／Z` 那段
...
- [ ] 材料清單裡有合成配方的材料旁邊有「+」；按下去在原地拆成下一階原料（框起來），自己變灰、按鈕變「−」；拆出來的原料數量要等於「無條件進位(需求/單次合成產出) × 該原料配方用量」（可以拿刷圖計畫同一個材料的合成步驟對一下數字，兩邊算法應該一致）；原料自己有配方的話也有「+」可以再拆；收合只影響自己這一層跟底下已經拆開的，其他材料不受影響；只有展開狀態才看得到「+」「−」跟拆開的框，收合狀態（72px）不顯示
...
跟上面的「合成表」彈窗是兩條不同的路徑，各自獨立：彈窗是「不影響清單本身、另開小窗看配方」，這個是「直接把清單裡那個材料換成它的原料，原地展開」，方便一路點到底看最底層原材料怎麼刷。

- `detail.expandedMaterials`：一個 `Set<materialId>`，記錄哪些材料目前被拆開。是扁平的（不分「在哪個父節點底下拆開」），所以同一個材料不管出現在哪個父節點下面，展開狀態都共用——這是刻意的簡化，這個遊戲的合成鏈同一個材料本來就只有一種拆法，沒有「依情境拆不同配方」的問題。
...
材料清單版面也跟著調整：一個材料被拆開後，本體（灰色那顆）改成站滿一行＋置中（`.mat-chip.dimmed { justify-content:center }`，同時把 `.mat-chip-label` 的 `flex` 從 `1 1 auto` 蓋成 `0 0 auto`，不然置中沒有用——label 本來會撐滿剩餘空間，`justify-content` 就沒東西好置中了）；拆出來的原料一欄直排、靠左對齊。每一層拆開的框（`.mat-node-frame`，用 `depth-N` class 上不同底色，借用職業色 `--PIONEER`／`--CASTER`／`--SUPPORT`／`--SNIPER` 配 `color-mix` 調淡，不是另外定義一組新顏色）**包住灰色本體＋它拆出來的原料**，不是只框原料——這是使用者看過第一版（框只包原料）之後又提出的調整，說明拆解框的視覺規則還在演化，改這塊時留意一下最新反饋是哪個版本。

### 拆解框寬度：從「每層砍半」改成線性遞減

原料寬度原本是每層固定 `width:50%`——但因為每一層都是巢狀在上一層裡面，`50%` 是相對「上一層自己的寬度」算的，疊加起來變成 1/2、1/4、1/8...使用者反應「縮太快了」。改成線性遞減：先用 `python` 對著已建置的 `app-data.json` 算過 `data.materials` 實際最深的合成鏈是幾層（遞迴 `craft.costs` 算深度，取所有材料的最大值），**目前資料是 4 層**，不是憑感覺猜的。定出「相對整個材料清單列寬」的目標序列 90% / 77% / 63% / 50%（第 1 層到第 4 層，線性遞減、第 4 層剛好半寬），因為 CSS `width` 百分比是相對**直接父層**算的，所以每一層實際寫的百分比是「這層目標 ÷ 上一層目標」：90%、85.6%、81.8%、79.4%（見 `app.css` 裡 `.mat-node-children.depth-N` 那幾行的註解換算）。`materialNode` 的 `frameDepth`（`Math.min(depth+1, 4)`）同時決定框的底色跟這個寬度 class，兩個視覺線索（顏色、寬度）永遠對應同一層，不會脫勾。深度真的超過 4 層的話（目前資料沒有，但以後可能有）會直接沿用第 4 層的樣式，不會報錯，只是視覺上就不會再變窄了——真的遇到這種情況要修的話，先重跑上面那段深度統計腳本確認新的最大深度，再回頭調這組數字。

同時把「原料固定半寬」的規則拿掉了（那是上一版的做法，被這次的線性遞減取代）。

### 掉落機率欄位改用企鵝物流期望值，拿掉遊戲拆包的機率標籤

材料明細關卡表的第二欄，原本是 `occPer` 對應的中文標籤＋顏色（固定掉落／常見掉落…），使用者覺得那個標籤常常是「—」（拆包沒登記，見上面那段解釋），不如直接寫企鵝物流的數字。改法：
- `dropTableHtml` 不再用 `OCC_PER` 字典，改成當場算 `apCost / apPerItem`（等於這個關卡這個材料的單次掉落期望值，跟 `stageDrops`／關卡懸浮提示用的是同一種算法，只是換了個角度算），欄位標題也改成「掉落量/次」。
...
### 使用者實測後的修正：技能等級按鈕看起來卡住

**真的有 bug**：`syncSkillPanel` 算目前該反白哪顆按鈕，邏輯是「這個技能的專精 > 0 就一定顯示專精，否則才顯示共用等級」（`mastery > 0 ? clamp(6+mastery,...) : clamp(skillLevelShared-1,...)`）。問題是 `createDetailState` 預設每個技能的 `mastery` 就是 3（不是 0！），所以**一進頁面** `mastery` 就已經 >0 了——這代表在使用者第一次去動任何技能等級按鈕之前，「專精優先於共用等級」這條規則就已經生效，點 1-7 的任何等級按鈕都只會改到 `detail.skillLevelShared`（材料清單確實有跟著變，因為材料計算是分開讀 `skillLevelShared` 的），但反白邏輯永遠先看 `mastery`，不會因為 `skillLevelShared` 變了就換人反白。使用者形容「點了 6 材料有改但方框還是專精 1」，完全對得上。

修法：點 1-7 的等級按鈕時，**順便把這個技能自己的 `mastery` 清成 0**。理由是遊戲機制上，專精本來就是「先到等級 7、再往上專精」的單向流程，選一個 ≤7 的等級在語意上就是「這個技能現在沒有專精」，讓 UI 反白邏輯跟這個語意一致。改完後點 1-7 一定會換成對應等級反白，點專精按鈕才會換回專精反白——兩邊互斥，不會再有「點了沒反應」的錯覺。`currentSkillLevel()`（範圍面板讀技能範圍用的）沒有另外修，因為它只是讀 `detail.mastery`／`detail.skillLevelShared`，狀態本身修對了，這裡自然就對。

### 使用者實測後的修正：養成狀態摘要要顯示技能

`planSummaryText` 原本只顯示精英化／等級，技能只有在 `current`／`target` 的 `skillLevelShared` 不同時才用一段附加文字帶出來。改成技能永遠顯示，格式併進主字串（`精X／LvY／Z`）。因為 `mastery` 是每個技能各自的陣列、不是單一數字，沒辦法直接塞進一個字串位置，用 `skillDisplay()` 簡化：**這個快照裡所有技能中最高的專精**，>0 就顯示羅馬數字（Ⅰ/Ⅱ/Ⅲ），否則顯示 `skillLevelShared` 本身。這是有意的簡化——如果之後真的有人設計出「同一個快照裡技能專精刻意不一樣」的情境，這裡的摘要文字只會顯示最高的那個，不會逐一列出每個技能，但目前場景（`預覽初始`／`設為滿級` 都是全部技能一致）不會踩到這個簡化的邊界。

### 使用者實測後的修正：技能等級「共用」的語意，以及後續兩輪 UI 調整

使用者原話：「現在所有技能的等級不是共用的，但共用就好，包含普攻你也可以加上去讓他有等級的欄位可以選取」、「現在沒有共用變成我設定練技能等級的時候它有時候顯示沒有抓到我設定的等級」。

**根因**：`createDetailState` 每個技能的 `mastery` 預設就是 3（不是 0），而 `syncSkillPanel` 判斷該反白哪顆按鈕的公式是「這個技能專精 > 0 就一定顯示專精」。這條規則本身沒錯（見上面「技能等級按鈕看起來卡住」那次修正），但它是**逐分頁**判斷的——切到另一個技能分頁時，那個分頁自己的 `mastery` 預設還是 3，公式一樣會判定「顯示專精」，於是使用者會看到「明明剛剛才點了共用等級，切個分頁又跳回顯示專精」，感覺共用沒生效。其實 `skillLevelShared` 真的有正確地共用、正確地寫入了，這單純是「同一顆按鈕列，兩種語意（共用 vs 各自獨立）疊在一起顯示」造成的觀感問題。

這次一路改了三輪，過程和最後定案都記錄下來，因為中間兩輪的方向都被使用者否決過，之後不要重踩：

**第一輪（否決）：拆成兩排**——嘗試把「共用等級 1-7」跟「專精 I-III」拆成兩個獨立的 `<div class="skill-levels">`（`skill-levels-shared`／`skill-levels-mastery`），專精那排多加一顆「無」按鈕（值 0，對應清空專精）。使用者反饋：「你改出bug了，技能1~7然後專精123這些原本是一排好好的，現在7之後換行了，還多了一個無」——因為兩個 `.skill-levels` 容器堆疊、外觀上就是「看起來是同一排但换行了」，「無」也是這次新增、之前沒有的按鈕，等於引入了兩個新的視覺問題。

**第二輪（過渡）：合併回一排，但普攻分頁停用專精按鈕**——把 10 顆按鈕（1-7 + 專Ⅰ/Ⅱ/Ⅲ，跟改動前一模一樣的排版）放回同一個 `.skill-levels` 容器，恢復成改動前的樣子；專精按鈕在切到普攻分頁時用 `disabled` 屬性鎖住（因為當時邏輯是 mastery 綁在 `detail.skillIndex`，普攻的 `skillIndex === -1` 沒有合法的技能可以寫入）。這輪解決了「換行＋多按鈕」的視覺 bug，但使用者接著說：「no need to make it safe no-op...讓技能專精123也都是隨時可以點去修改，有時候使用者就是不想要切到技能頁面再去改專精，他們一定知道自己在改甚麼」——也就是普攻分頁下專精按鈕被鎖住這件事本身也不是使用者要的。

**最終定案**：新增 `detail.masteryIndex`（`createDetailState` 預設 0，操作員沒有技能則是 `BASIC_SKILL`）。`onSkill(i)` 每次被呼叫，只要 `i >= 0` 就同步 `detail.masteryIndex = i`；但切到普攻（`i === BASIC_SKILL`）時**不會**把 `masteryIndex` 改回 -1——這是跟 `skillIndex` 唯一的差異，也是整個設計的關鍵：`skillIndex` 決定「畫面下半部顯示哪個技能的說明文字」，`masteryIndex` 決定「專精按鈕列現在對應哪個技能」，兩者平常同步變化，只有在普攻分頁時會分岔（`skillIndex = -1` 但 `masteryIndex` 留著最後一次造訪的技能）。`onMastery(n)` 一律寫 `detail.mastery[detail.masteryIndex]`，不再檢查 `skillIndex`。`syncSkillPanel` 的按鈕反白／`disabled` 也全部改成讀 `masteryIndex`（透過 `refs[detail.masteryIndex]` 查該技能是否真的有專精，`hasMastery = skill.levels.length > 7`），不再對「現在是不是普攻分頁」做特殊處理——普攻分頁下專精按鈕能不能點，純粹取決於「`masteryIndex` 指到的那個技能有沒有專精」，跟目前開哪個分頁無關。

效果：使用者可以留在普攻分頁（例如想順便看一下範圍或特性說明），同時直接點右上角的專精按鈕改某個技能的專精，不用切過去。改完後如果切到 S1-S3 任一分頁，`masteryIndex` 會跟著切換成那個分頁；再切回普攻，專精列會顯示「最後造訪的那個技能」的專精狀態並保持可編輯——這是刻意的行為，不是「殘留上一個技能沒清乾淨」的 bug，往後如果有人想改這個行為（例如普攻分頁固定顯示 S1），記得同時看一下 `onSkill`／`syncSkillPanel` 兩處都要一起改，兩邊現在是靠 `masteryIndex` 這個共同的字串來源對齊的。

`planSummaryText` 也照使用者要求改回「技能分開寫在後面」的格式（跟這個功能最早期的樣子一樣）：`精0／Lv50→精2／Lv80　技能：Ⅲ→Ⅲ`，不再嘗試把技能併進 `精X／LvY／Z` 那段主字串。`skillDisplay()` 本身沒有改（還是「這個快照裡所有技能中最高的專精，>0 就顯示羅馬數字，否則顯示共用等級」那條既有簡化邏輯），只是呼叫端的字串組裝方式改了。

### 使用者實測後的修正：材料面板標題列的置中

`.mat-dock-head` 原本是「一行 padding 8px 6px，裡面塞文字＋一顆 22×22 的小按鈕，靠 `align-items:center` 置中」，數學上量起來是置中的，但使用者反應視覺上「黏在天花板上」。改成更保守的做法：不靠 flex 置中去猜，直接把收合按鈕做成**貼右邊、跟頭部同高的正方形格子**（`align-items:stretch` + 固定寬度 `38px` + `border-left` 當分隔線），按鈕內部再用 `flex;align-items:center;justify-content:center` 置中圖示——按鈕本身的框永遠跟頭部一樣高，置中與否不會再取決於「頭部的 padding 抓不抓得準」。手機版（`max-width:720px`）那條收合狀態的 media query 原本會覆寫 `flex-direction:column`，但沒覆寫新加的 `height:38px`，會導致手機底部橫條的頭部區塊被夾成固定 38px（跟橫條本身 56px 對不齊），一起加了 `height:auto` 蓋掉。
