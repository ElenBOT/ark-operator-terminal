...
- 檔案頁範圍面板下方有「設為初始／編輯當前／編輯目標／設為滿級」按鈕列。「編輯當前」「編輯目標」是有記憶的分頁式切換：按下後左側精英化／等級／技能等級／專精旋鈕改成即時編輯那一份養成狀態，調哪個旋鈕就直接改那份，不用再按一次；再按一次同一顆分頁鈕會關閉編輯（回到單純預覽，不寫入任何一份）。「設為初始」「設為滿級」在有編輯中的分頁時也會直接寫入那一份（滿級的潛能維持潛 1，因為潛能不吃材料）。預設「當前」「目標」都是滿級，所以材料清單一開始是空的；養成狀態摘要會列出精英化／等級，技能另外分開列在後面（例如「精0／Lv50→精2／Lv80　技能：Ⅲ→Ⅲ」）。視窗右側材料面板預設窄版直列，可展開成橫式並換行、上半列材料、下半共用一塊「取得方式」面板；點材料才會把關卡表（依理智期望值排序、標最優）或加工站合成配方填進那塊共用面板
- 技能面板的等級列是單一一排（1–7 再接專Ⅰ–Ⅲ，共 10 顆），不管目前開哪個分頁（含普攻）都看得到、都能點：1–7 是所有技能共用的等級（真實遊戲機制），專精是每個技能各自獨立的——但**專精按鈕永遠是可以點的**，就算目前開著普攻分頁；它記著「最後一次切到的技能分頁」當作目前要改哪個技能的專精，不用特地切過去才能改
