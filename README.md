...
- 檔案頁範圍面板下方有「設為初始／編輯當前／編輯目標／設為滿級」按鈕列。「編輯當前」「編輯目標」是有記憶的分頁式切換：按下後左側精英化／等級／技能等級／專精旋鈕改成即時編輯那一份養成狀態，調哪個旋鈕就直接改那份，不用再按一次；再按一次同一顆分頁鈕會關閉編輯（回到單純預覽，不寫入任何一份）。「設為初始」「設為滿級」在有編輯中的分頁時也會直接寫入那一份（滿級的潛能維持潛 1，因為潛能不吃材料）。預設「當前」「目標」都是滿級，所以材料清單一開始是空的；養成狀態摘要會列出精英化／等級／技能（技能有專精時顯示羅馬數字，例如「精2／Lv90／Ⅲ」）。視窗右側材料面板預設窄版直列，可展開成橫式並換行、上半列材料、下半共用一塊「取得方式」面板；點材料才會把關卡表（依理智期望值排序、標最優）或加工站合成配方填進那塊共用面板
...
- 材料明細標題「合成表」旁邊還有一顆「刷圖計畫 ›」，只針對這一個材料排刷圖／合成計畫（跟全域那顆「刷圖計畫」邏輯共用，只是只算這一個材料）。彈窗一開始預設要「目前差額顯示的全部數量」，但上面有個數量調整（−／輸入框／＋），可以手動往下調（例如自己已經有一部分了，只想知道剩下的怎麼刷），最多不會超過原本需要的數量
